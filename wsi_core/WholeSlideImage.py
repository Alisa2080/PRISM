...
class WholeSlideImage(object):
    def __init__(self, path, slide_id):
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        # process small size patch
        if self.level_dim[seg_level][0] < 800:
            k = 1200/self.level_dim[seg_level][0]
            logk = int(np.log2(k))
        #print(seg_level)
        #print(self.level_dim[seg_level])
            img = np.array(self.wsi.read_region((0,0), seg_level-logk, self.level_dim[seg_level-logk]))
        else:
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        save_path_hdf5 = os.path.join(save_path, self.name + '.h5')
        print("Creating patches for: ", self.name, "...",)
        elapsed = time.time()
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        #coord_candidates = coord_candidates.astype(np.int32)
        #print(coord_candidates.dtype)
        # print('coord_candidates:',coord_candidates.shape)
        # print('cont_check_fn:',contour_fn)
        ...
        print('====ref_patch:',ref_patch_size[0])
    ...
    def visHeatmap(self, scores, coords, 
                   use_roi=False,
                   vis_level=-1, 
        ...
        Args:
            ...
            use_roi: whether visualize one ROI region or the whole slide image
        ...
        # Ensure vis_level doesn't exceed available levels
        max_level = len(self.level_downsamples) - 1
        if vis_level > max_level:
            print(f'Warning: vis_level {vis_level} exceeds max level {max_level}, using level {max_level}')
            vis_level = max_level
        ...
        if use_roi:
            if top_left is not None and bot_right is not None:
                region_size = self.level_dim[vis_level]
                bot_right_whole = self.level_dim[0]


                top_left_pad = (max(top_left[0]-4096,0),max(top_left[1]-4096,0))
                bot_right_pad = (min(bot_right[0]+4096,bot_right_whole[0]),min(bot_right[1]+4096,bot_right_whole[1]))

                scores, coords = screen_coords(scores, coords, top_left_pad, bot_right_pad)
                coords = coords - top_left_pad
                top_left_pad = tuple(top_left_pad)
                bot_right_pad = tuple(bot_right_pad)
                w, h = tuple((np.array(bot_right_pad) * scale).astype(int) - (np.array(top_left_pad) * scale).astype(int))
                w_o,h_o = tuple((np.array(bot_right) * scale).astype(int) - (np.array(top_left) * scale).astype(int))
                w_pad,h_pad = tuple((np.array(top_left) * scale).astype(int) - (np.array(top_left_pad) * scale).astype(int))
                region_size = (w, h)

            else:
                region_size = self.level_dim[vis_level]
                top_left_pad = top_left = (0,0)
                bot_right_pad =  bot_right = self.level_dim[0]
                w, h = region_size
                w_o,h_o = w,h
                w_pad,h_pad = (0,0)
        else:
        ...
        if segment:
            if use_roi:
                tissue_mask = self.get_seg_mask(region_size, scale, use_holes=use_holes, offset=tuple(top_left_pad))
            else:
        ...
        if not blank_canvas:
            ...
            if use_roi:
                img = np.array(self.wsi.read_region(top_left_pad, vis_level, region_size).convert("RGB"))
            else:
                img = np.array(self.wsi.read_region(top_left, vis_level, region_size).convert("RGB"))
        else:
            # use blank canvas
        ...
        if use_roi:
            img = img[w_pad:w_pad+w_o,h_pad:h_pad+h_o]
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0,0)):
        ...
        # Safety check: ensure contours_tissue and holes_tissue are initialized
        if self.contours_tissue is None:
            print('Warning: contours_tissue is None, creating default mask')
            # Return a mask with all tissue (for visualization when no segmentation available)
            tissue_mask = np.full(np.flip(region_size), 1).astype(bool)
            print('detected {}/{} of region as tissue (default: all tissue)'.format(tissue_mask.sum(), tissue_mask.size))
            return tissue_mask
        
        if len(self.contours_tissue) == 0:
            print('Warning: contours_tissue is empty, creating default mask')
            # Return a mask with all tissue (for visualization when no contours found)
            tissue_mask = np.full(np.flip(region_size), 1).astype(bool)
            print('detected {}/{} of region as tissue (default: all tissue)'.format(tissue_mask.sum(), tissue_mask.size))
            return tissue_mask
        ...
        # Safety check for holes_tissue
        if self.holes_tissue is None:
            self.holes_tissue = [[] for _ in range(len(self.contours_tissue))]

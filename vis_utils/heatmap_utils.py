...
from datasets.vis_dataset import Wsi_Region
import h5py
...
def drawHeatmap(scores, coords, slide_path=None, wsi_object=None, vis_level = -1, **kwargs):
    ...
    # Handle both WholeSlideImage and direct OpenSlide objects
    if hasattr(wsi_object, 'getOpenSlide'):
        # wsi_object is a WholeSlideImage instance
        wsi = wsi_object.getOpenSlide()
        if vis_level < 0:
            vis_level = wsi.get_best_level_for_downsample(32)
        
        # Ensure vis_level doesn't exceed available levels
        max_level = wsi.level_count - 1
        ...
        # wsi_object is a direct OpenSlide instance - need to wrap it in WholeSlideImage
        if slide_path is None:
            raise ValueError("slide_path must be provided when wsi_object is a direct OpenSlide instance")
        
        slide_id = os.path.splitext(os.path.basename(slide_path))[0]
        # Create a proper WholeSlideImage wrapper
        temp_wsi_object = WholeSlideImage(slide_path, slide_id)
        
        if vis_level < 0:
            vis_level = temp_wsi_object.wsi.get_best_level_for_downsample(32)
        
        # Ensure vis_level doesn't exceed available levels
        max_level = temp_wsi_object.wsi.level_count - 1
        if vis_level > max_level:
            print(f'Warning: vis_level {vis_level} exceeds max level {max_level}, using level {max_level}')
            vis_level = max_level
            
        heatmap = temp_wsi_object.visHeatmap(scores=scores, coords=coords, vis_level=vis_level, **kwargs)
    
    return heatmap

def initialize_wsi(slide_id, wsi_path, seg_mask_path=None, seg_params=None, filter_params=None):
    wsi_object = WholeSlideImage(wsi_path,slide_id)

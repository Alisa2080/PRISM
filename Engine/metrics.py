...
def get_cls_metrics(args,logits,labels,bootstrap,device,raw=False):
    ...
    if args.num_classes == 2 and args.bin_metric:
         logits = logits[:,1]
...
def metrics_base(args,bootstrap=False,device=None,raw=False):
    if args.task == 'survival':
        ...
        if args.num_classes == 2 and args.bin_metric:
            metrics: Dict[str, Metric] = {
                    "Acc": Accuracy(top_k=1, num_classes=int(args.num_classes), task='binary').to(device),
                    "F1": F1Score(num_classes=int(args.num_classes),task='binary').to(device),
                    "AUC": AUROC(num_classes=int(args.num_classes),task='binary').to(device),
                    "Precision": Precision(num_classes=int(args.num_classes),task='binary').to(device),
                    "Recall": Recall(num_classes=int(args.num_classes),task='binary').to(device),
                    "CK": CohenKappa(num_classes=int(args.num_classes),task='binary').to(device),
                    "Acc_micro": Accuracy(top_k=1, num_classes=int(args.num_classes), task='binary').to(device),
            }
        else:
            metrics: Dict[str, Metric] = {
                    "Acc": Accuracy(top_k=1, num_classes=int(args.num_classes), task='multiclass',average='macro').to(device),
                    "F1": F1Score(num_classes=int(args.num_classes), average="macro", task="multiclass").to(device),
                    "AUC": AUROC(num_classes=int(args.num_classes), average="macro", task="multiclass").to(device),
                    "Precision": Precision(num_classes=int(args.num_classes), average="macro", task="multiclass").to(device),
                    "Recall": Recall(num_classes=int(args.num_classes), average="macro", task="multiclass").to(device),
                    "CK": CohenKappa(num_classes=int(args.num_classes),task="multiclass").to(device),
                    "Acc_micro": Accuracy(top_k=1, num_classes=int(args.num_classes), task='multiclass').to(device),
            }
...
def get_metric_val(args, bag_logit, bag_labels, model, status, early_stopping, epoch,
                    loss_cls_meter, suffix=None,surv=False):

...
from Engine.utils import patch_shuffle
...
class BaseTrainer():
    ...
    def train(self, args, model, model_ema, loader, optimizer, device, amp_autocast, criterion, loss_scaler, scheduler,epoch, model_others):
        ...
        self.engine.init_func_train(args, model=model, others=model_others, epoch=epoch, optimizer=optimizer)
        optimizer.zero_grad()
        update_sample_count = 0

        for batch_idx, batch in enumerate(loader):
            last_batch = batch_idx == last_batch_idx
            need_update = last_batch or (batch_idx + 1) % args.accumulation_steps == 0
            update_idx = batch_idx // args.accumulation_steps
            ...
            batch = {
                k: ([vv.to(device=device, non_blocking=True) for vv in v] if isinstance(v, list) and k in ['input', 'path_features', 'omic_list'] else
                    v.to(device=device, non_blocking=True) if isinstance(v, torch.Tensor) else v)
                for k, v in batch.items()
            }
            

            # 新格式：多模态数据（path_features + omic_list）
            bag = batch.get('path_features', None)
            omic_data = batch.get('omic_list', None)
            ...
            self.engine.after_get_data_func(args, device=device, bag=bag, omic_data=omic_data,optimizer=optimizer, loader=loader,
                                            n_iter=epoch * len(loader) + batch_idx, epoch=epoch, model=model,
                                            others=model_others)
            ...
            def _forward():
                with amp_autocast(device_type='cuda', dtype=torch.float16, enabled=args.amp):
                    ...
                    with profile_env() as prof:
                        logits, labels, aux_loss, batch_loss,patch_num, keep_num, pad_ratio,kn_std = self.engine.forward_func(args,
                        model=model,
                        model_ema=model_ema,
                        bag=bag,
                        omic_data=omic_data,
                    ...
                    if logits is None:
                        return None, None, None, None,patch_num, keep_num, pad_ratio,kn_std
                    # continue
                    bs = logits.size(0)
                    logit_loss = criterion(logits.view(bs, -1), labels.view(bs))
                    loss = args.main_alpha * logit_loss + aux_loss * args.aux_alpha + batch_loss * args.batch_loss_alpha

                    loss /= accum_steps
                    return loss, aux_loss, logit_loss, batch_loss, patch_num, keep_num, pad_ratio,kn_std
            ...
            if has_no_sync and not need_update:
                with model.no_sync():
                    loss, aux_loss, logit_loss, batch_loss,patch_num, keep_num, pad_ratio,kn_std = _forward()
                    if loss is None:
                        continue
                    _backward(loss)
            else:
            ...
            loss_batch_meter.update(batch_loss * accum_steps * args.batch_loss_alpha, batch_size)
            ...
            if update_idx % args.log_iter == 0 or batch_idx == len(loader) - 1:
                ...
                rowd = OrderedDict([
                    ...
                    ('batch_loss', loss_batch_meter.avg),
                ...
                if 'abmilx' in args.model and args.abx_attn_plus:
                ...
                if loss_batch_meter.val != 0:
                    loss_batch_meter_avg_format = f'BatchLoss: {loss_batch_meter.val:#.4g} ({loss_batch_meter.avg:#.16g})  '
                else:
                    loss_batch_meter_avg_format = ''
    ...
    def validate(self, args, model, loader, device, criterion, amp_autocast, early_stopping=None, epoch=None,
        ...
        self.engine.init_func_val(args, status=status, amp_autocast=amp_autocast, model=model, loader=loader,
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                self.engine.after_get_data_func(args, device=device, bag=bag, optimizer=None, loader=loader,
                                                n_iter=None, epoch=None, model=model)
    ...
    def surv_train(self, args, model, model_ema, loader, optimizer, device, amp_autocast, criterion, loss_scaler,
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            def _forward():
                with amp_autocast(device_type='cuda', dtype=torch.float16, enabled=args.amp):
                    logits, labels_surv, aux_loss, batch_loss,patch_num, keep_num, pad_ratio,kn_std = self.engine.forward_func(
                        args,
                        model=model,
                        model_ema=model_ema,
                        bag=bag,
    ...
    def surv_validate(self, args, model, loader, device, criterion, amp_autocast, early_stopping=None, epoch=None,
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                self.engine.after_get_data_func(args, device=device, bag=bag, optimizer=None, loader=loader,
                                                n_iter=None, epoch=None, model=model, others=others)

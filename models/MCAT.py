...
class MCAT(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], num_classes=4,
                 model_size_wsi: str='small', model_size_omic: str='small', dropout=0.25,
                  task="survival", extract_scale="x20",max_rois=2000):
        ...
        print(f"[MCAT] 配置提取尺度: {extract_scale}")
        if extract_scale in self.scale_indices:
            start_idx, end_idx = self.scale_indices[extract_scale]
            print(f"[MCAT] 将提取索引 {start_idx}-{end_idx-1} 的特征 (共{end_idx-start_idx}个patch)")
    ...
    def forward(self, **kwargs):
        ...
        if len(x_path.shape) == 4:
            ...
            if num_patches == 84 and self.extract_scale in self.scale_indices:
                ...
                # 兼容不同的输入格式
        if "x_omics" in kwargs:
            # 支持Ciallo风格的输入格式: x_omics是list
            x_omic = kwargs["x_omics"][:6]  # CMTA只支持前6个pathway
        else:
            # 支持原始CMTA格式: x_omic1, x_omic2, ...

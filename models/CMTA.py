...
from models.CMTA_util import initialize_weights
from models.CMTA_util import NystromAttention
from models.CMTA_util import BilinearFusion
from models.CMTA_util import SNN_Block
from models.CMTA_util import MultiheadAttention
...
class CMTA(nn.Module):
    def __init__(self, input_dim=1024,inner_dim=256,mil_bias=True,
                omic_sizes=[100, 200, 300, 400, 500, 600], dropout=0.25,
                num_classes=4, fusion="concat", model_size="small",max_patches=0):
        ...
        self.fusion = fusion
        self.size_dict = {
            "pathomics": {"small": [input_dim, inner_dim, inner_dim], "large": [input_dim, 512, inner_dim]},
            "genomics": {"small": [input_dim, inner_dim], "large": [input_dim, input_dim, input_dim, inner_dim]},
        }
        ...
        for idx in range(len(hidden) - 1):
            fc.append(nn.Linear(hidden[idx], hidden[idx + 1],bias=mil_bias))
            fc.append(nn.ReLU())
        ...
        for input_dim in omic_sizes:
            fc_omic = [SNN_Block(dim1=input_dim, dim2=hidden[0],bias=mil_bias)]
            for i, _ in enumerate(hidden[1:]):
                fc_omic.append(SNN_Block(dim1=hidden[i], dim2=hidden[i + 1], dropout=self.dropout,bias=mil_bias))
    ...
    def forward(self,x_path,omic_data,**kwargs):
        if x_path.shape[1] > self.max_patches and self.max_patches > 0:
            indices = torch.randperm(x_path.shape[1])[:self.max_patches]
            x_path = x_path[:, indices, : ]
        
        # 支持Engine传递的omic_data格式
        x_omic = omic_data[:6]  # CMTA只支持前6个pathway
        # Enbedding
        # genomics embedding
        genomics_features = [self.genomics_fc[idx].forward(sig_feat) for idx, sig_feat in enumerate(x_omic)]
        # 支持批量训练：将(6, batch_size, 256)转换为(batch_size, 6, 256)
        genomics_features = torch.stack(genomics_features).permute(1, 0, 2)  # [batch_size, 6, 256]
        # pathomics embedding
        pathomics_features = self.pathomics_fc(x_path)  # 保持3D: (1, num_patches, 256)
        # encoder
        # pathomics encoder
